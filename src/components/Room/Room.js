...
const room = (props) => {
    ...
    return (
        <li className={classs} key={props.id} onClick={(e) => props.select(e, props.index)}>
            ...
            <button className="room__button" onClick={(e) => {props.showRoomData(props.id, props.index)}}>Data</button>
            <button className="room__button" onClick={(e) => {props.showLocations(props.id, props.index)}}>Locations</button>
